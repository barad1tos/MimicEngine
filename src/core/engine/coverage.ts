...
export function computeCoverage(
  ...
  const discoveredHexes = new Set(palette.map((entry) => entry.hex));
  const mappedHexes = new Set(
    palette.filter((entry) => mapping.has(mappingKeyOf(entry))).map((entry) => entry.hex),
  );
  const discovered = discoveredHexes.size;
  const mapped = mappedHexes.size;
